...
This file is the maker/contributor doc for people extending this repo. The agent's own live system prompt is `.copilotstudio/agent/instructions.md` — paste that into Copilot Studio's Agent > Overview > Instructions; this file and that one should stay in sync, but this one is not auto-loaded by Copilot Studio.

## Default Model

Use the most capable generative AI model available in the agent's Copilot Studio model settings unless the user or task explicitly calls for a different model.
...
- When creating a new project, use `.copilotstudio/topics/new-project` or follow `templates/project_README.md` and `templates/PROJECT_AGENTS.md`.
- When creating a new person note, use `.copilotstudio/topics/new-person` or follow `people/person.md`.
- During Assistant onboarding, after scanning connected Teams/Slack, email, calendar, docs, project trackers, and GitHub context, proactively propose the people files and project packets that should be created or updated. Ask for approval, then write the approved files directly in this repo.
...
Topics in `.copilotstudio/topics/` are meant to be read and used in place. Do not assume they are installed globally or available outside this agent.
