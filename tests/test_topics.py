...
def test_topic_frontmatter_shape() -> None:
    topic_files = sorted((ROOT / ".copilotstudio" / "topics").glob("*/TOPIC.md"))
    assert topic_files
...
def test_onboarding_scripts_default_to_repo_root_vault() -> None:
    script_dir = ROOT / ".copilotstudio" / "topics" / "onboarding" / "scripts"
    for script_name in (
