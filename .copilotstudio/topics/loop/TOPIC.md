---
name: loop
description: "Set up a Recurrence trigger for this agent. Use when the user asks this agent to keep going, check again, follow up, retry, monitor, or resume on a recurring cadence."
...
Codex CLI's original `loop` skill attached a "heartbeat" automation directly to the live chat thread and could rename that thread to reflect status. Copilot Studio agents do not have a native "attach a recurring job to this specific conversation and rename it" tool — the closest real building block is a **Recurrence trigger** (an event trigger type) on the agent itself:

- In Copilot Studio: agent's Overview page > Triggers section > Add trigger > select the Recurrence trigger. It fires independently of any open conversation and delivers a payload the agent's instructions act on, on a cadence you set. This requires generative orchestration turned on for the agent, and in some tenants an admin must turn on "solution-aware cloud flow sharing" for the environment before Triggers shows up at all.
- If Triggers isn't available in this environment, use a Power Automate recurrence trigger instead: have the flow call this agent's channel API (e.g. Direct Line) or post to wherever the check-in should land (email, Teams channel, Dataverse row), on the desired schedule.
...
2. Decide whether a Recurrence trigger is useful. Use one when the work benefits from returning after time passes or external state may change; continue immediately instead when the task can simply be finished now.
...
5. Create the trigger in Copilot Studio (agent's Overview page > Triggers section > Add trigger > Recurrence) with:
   - the inferred name
...
- Treat "loop," "keep going," and "check again" as authorization to create the Recurrence trigger requested in that message.
