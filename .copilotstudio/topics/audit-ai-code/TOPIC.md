...
- "check this PR for AI code smells"
...
For larger diffs, parallelize read-only review into up to four passes: local idiom/reuse, cohesion/API shape, control-flow/exception boundaries, and behavior tests/safety/performance. Prefer a stronger model for ambiguous tradeoffs and a smaller model for narrow, easy-to-verify scans — in Copilot Studio terms, this means picking the strongest generative-AI model available in the agent's model settings for the ambiguous passes.
