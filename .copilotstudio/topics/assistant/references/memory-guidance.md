...
Copilot Studio agents have no built-in cross-conversation memory of their own. Only seed a durable-memory surface through a real, explicitly available tool (for example a connected Dataverse table, a Microsoft Graph-backed personalization surface, or this vault's knowledge source) — never by writing ad hoc files outside the vault as a substitute. If no durable-memory surface is available, keep the calibrated map as the chat baseline and say that plainly.

## Shared Memory Vault

The explicit shared-memory vault is the durable-memory surface for this
template; it is not a stand-in for a native agent memory feature, because none
is assumed here. When the user approves vault setup, Assistant may create or
